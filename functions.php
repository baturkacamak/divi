...
function print_thumbnail($thumbnail = '', $use_timthumb = true, $alttext = '', $width = 100, $height = 100, $class = '', $echoout = true, $forstyle = false, $resize = true, $post='', $et_post_id = '' ) {
	...
	if ( has_post_thumbnail( $et_post_id ) ) {
		...
		$et_attachment_image_attributes = wp_get_attachment_image_src( get_post_thumbnail_id( $et_post_id ), 'medium' );
	...
	if ( false === $forstyle ) {
		...
		$output .= ' srcset="'. wp_get_attachment_image_srcset(get_post_thumbnail_id( $et_post_id ), $et_size) . '" ';

		if ($class <> '') $output .= " class='" . esc_attr( $class ) . "' ";

		$dimensions = apply_filters( 'et_print_thumbnail_dimensions', " width='" . esc_attr( '300' ) . "' height='" .esc_attr( '200' ) . "'" );
